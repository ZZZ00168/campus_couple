...
class UserPostFollowList:  # 获取关注用户的文章列表
    def POST(self):  # 传入access_token,user_id,start_poost_id,start_index,end_index
        ...
        try:
            ...
            results = db.select('follow,post', vars={'user_id': input.user_id, 'post_id': input.start_post_id,
                                                     "post_id_number": post_id_number},
                                where="post.post_id>$post_id and follow.user_id=$user_id and follow.followed_id=post.user_id",
                                what='post.user_id as post_user_id,add_time,content,post_id', order='post_user_id desc',
